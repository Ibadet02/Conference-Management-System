          ...
          >
            {userAppliedProjectsData?.length > 0 ? (
              <>
              {
                type==="New" ?
                <>
                {userAppliedProjectsData?.filter((e) => !submissions?.some((a) => a?.projectId === e?.projectId))
?.map((project) => {
                  return (
                    <AppliedProjectData
...
?.length === 0 &&
            ...
            >
              No Data
                ...
                :
                <>
                {userAppliedProjectsData?.filter((e) => submissions?.some((a) => a?.projectId === e?.projectId))?.map((project) => {
                  const matchedItem=submissions?.find((e) => e?.projectId === project?.projectId);
                  return (
                    <AppliedProjectData
                      key={project.projectId}
                      projectData={project.userAppliedProject}
                ...
                {userAppliedProjectsData?.filter((e) => submissions?.some((a) => a?.projectId === e?.projectId))?.length === 0 &&
